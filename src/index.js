...
function update() {
  ...
  //the animation rate of the flamingo
...
function loadModels() {
  ...
  //a reusable function to set up the models. were passing in position parameter
  //so that they can be indivdually placed around the scene
  const onLoadAnimation = function (gltf, position) {
    ...
    const animation = gltf.animations[0]; //get the first animation from the gltf and assign it to a varible call animation
  ...
  const onLoadStatic = function (gltf, position) {
    ...
    robot.position.copy(position); //copy the position from the load function call

    modelLoaded = true; //once our model has loaded, set our modelLoaded boonlean flag to true
  ...
  //the loader will send any error messages to the function and, we'll log
  //them to console
